...
exports.postOnChainDiGiProfileManager = void 0;
...
const postOnChain = async (request) => {
    const result = await apollo_client_1.apolloClient.mutate({
        mutation: generated_1.PostOnchainDocument,
        variables: {
            request,
        },
    });
...
const postOnChainDiGiProfileManager = async () => {
    const address = (0, ethers_service_1.getAddressFromSigner)();
    console.log('post onchain digi profile manager: address', address);
    await (0, login_1.login)(address);
    const ipfsResult = await (0, ipfs_1.uploadIpfs)(publication_metadata_mocks_1.publicationMetadataTextOnly);
    console.log('post onchain digi profile manager: ipfs result', ipfsResult);
    const request = {
    ...
    console.log('post onchain digi profile manager: result', result);
    await (0, wait_until_complete_1.waitUntilDiGiManagerTransactionIsComplete)(result, 'unblock');
};
exports.postOnChainDiGiProfileManager = postOnChainDiGiProfileManager;
(async () => {
    if ((0, config_1.explicitStart)(__filename)) {
        await (0, exports.postOnChainDiGiProfileManager)();
    }
})();
