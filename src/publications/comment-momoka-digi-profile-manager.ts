...
const createMomokaCommentWithDiGiManager = async (request: MomokaCommentRequest) => {
...
const createCommentOnMomoka = async (createMomokaCommentRequest: MomokaCommentRequest) => {
  ...
  console.log(
    'create momoka comment via digi-manager: createMomokaCommentWithDiGiManager',
    dispatcherResult
  );
...
export const commentOnMomokaDiGiProfileManager = async () => {
...
(async () => {
  if (explicitStart(__filename)) {
    await commentOnMomokaDiGiProfileManager();
  }
})();
